...
export async function POST(req: Request) {
  ...
  // 9. 브랜드 콘텐츠(content_posts + content_stats) 생성
  let contentPostCount = 0
  let contentStatCount = 0

  try {
    const clinicContent: Record<string, { titles: string[]; thumbBase: string }> = {
      [c1.id]: {
        titles: ['쌍꺼풀 수술 전후 비교', '코 성형 회복 과정', '자연스러운 눈 성형 브이로그', '성형 Q&A 라이브', '안검하수 교정 후기'],
        thumbBase: 'mirae',
      },
      [c2.id]: {
        titles: ['레이저 토닝 효과 비교', '피부과 시술 종류 소개', '여드름 흉터 제거 전후', '피부 관리 루틴 공개', '보톡스 시술 브이로그'],
        thumbBase: 'kangnam',
      },
      [c3.id]: {
        titles: ['임플란트 시술 과정', '치아 미백 전후 비교', '교정 완성 후기 인터뷰', '스케일링 Q&A', '라미네이트 변신 사례'],
        thumbBase: 'seoul',
      },
    }
    ...
    const utmCampaigns: Record<string, string> = {
      youtube: '2026_brand_youtube',
      instagram_feed: '2026_brand_insta_feed',
      instagram_reels: '2026_brand_insta_reels',
      tiktok: '2026_brand_tiktok',
      naver_blog: '2026_brand_blog',
    }
      ...
      for (const { platform, count } of contentPlatforms) {
        for (let i = 0; i < count; i++) {
          ...
          allPosts.push({
            ...
            thumbnail_url: null,
            published_at: new Date(Date.now() - publishedDaysAgo * 24 * 60 * 60 * 1000).toISOString(),
    ...
    if (insertedPosts && insertedPosts.length > 0) {
      ...
      for (const post of insertedPosts) {
        ...
        for (let d = 0; d < 7; d++) {
          const date = new Date(Date.now() - d * 24 * 60 * 60 * 1000)
          const dateStr = date.toISOString().split('T')[0]
          const decay = 1 - d * 0.05
          statsRows.push({
            post_id: post.id,
            stat_date: dateStr,
            views: Math.floor(base * decay * (0.8 + Math.random() * 0.4)),
            likes: Math.floor(likeBase * decay * (0.8 + Math.random() * 0.4)),
            comments: Math.floor(likeBase * 0.1 * decay * (0.8 + Math.random() * 0.4)),
            shares: Math.floor(likeBase * 0.05 * decay),
            saves: Math.floor(likeBase * 0.08 * decay),
          })
        }
      }
    ...
    console.warn('[Seed] content_posts/content_stats 테이블이 없거나 오류:', e)
  }
