...
- [ ] **0.1** Supabase 대시보드에서 `clinic_api_configs` 테이블의 **실제 스키마** 확인
  - 컬럼 목록, 타입, NOT NULL 제약, DEFAULT 값
  - UNIQUE 제약 (`clinic_id, platform` 여부)
  - 인덱스 목록
  - 기존 데이터가 있다면 샘플 추출 (platform 값 목록)
- [ ] **0.2** Supabase 대시보드에서 `ad_campaign_stats` 테이블의 **UNIQUE 제약** 확인
  ...
  ```
- [ ] **0.3** 결과에 따라 Phase 1 마이그레이션 SQL 확정 (CREATE TABLE vs ALTER TABLE)
...
- [ ] **1.2** `supabase/migrations/20260319_ad_campaign_stats_unique.sql` 생성
...
- [ ] **1.3** `lib/crypto.ts` 생성 — API 키 암호화/복호화
...
- [ ] 암호화 → 복호화 라운드트립 정상
- [ ] 기존 meta_capi, youtube_content, instagram_content 설정에 영향 없음
...
- [ ] 마스킹 확인 (GET 응답에 토큰 전체 노출 안 됨)
- [ ] platform 화이트리스트 검증 (meta_ads/google_ads/tiktok_ads 외 거부)
...
- [ ] 기존 병원 관리 기능(생성/수정/알림설정/활성화) 회귀 없음
- [ ] 모바일 반응형 확인
...
### Quality Gate ✋
- [ ] `npm run build && npm run lint` 통과
- [ ] clinic_api_configs에 설정된 병원: 해당 API 키로 동기화, ad_campaign_stats에 clinic_id 저장
- [ ] clinic_api_configs에 미설정 + 환경변수 있는 경우: 환경변수 폴백 동작
- [ ] 양쪽 모두 없으면: 해당 매체 스킵 (count=0, 에러 없음)
- [ ] 수동 동기화("지금 데이터 수집")가 선택된 병원의 API 키로 동작
- [ ] `ads/stats` API에 clinic_id 필터 적용 확인
- [ ] Cron maxDuration=300 설정 확인
...
- `clinic_api_configs` 테이블은 이미 meta_capi, youtube_content, instagram_content에서 사용 중 (metaCapi.ts, content/sync/route.ts). 마이그레이션 파일은 없으나 DB에 존재 추정 → Phase 0에서 확인 필수
- `ad_campaign_stats`에 `clinic_id` 컬럼 이미 존재 (시드 데이터에서 확인). UNIQUE 제약에는 미포함 → Phase 1에서 변경
- 현재 동기화 서비스(metaAds/googleAds/tiktokAds)는 환경변수만 사용하고 clinic_id를 upsert에 포함하지 않음 → Phase 4에서 수정
- 현재 `ads/stats` API에 clinic_id 필터 없음 → 멀티테넌트 격리 위반, Phase 4.7에서 수정
- metaCapi.ts의 `getCapiConfig()` 패턴이 환경변수 폴백의 검증된 레퍼런스
